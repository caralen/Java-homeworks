...
public class Lexer {
	...
	private boolean checkIfString() {
		...
		while(counter < data.length) {
			if(data[counter] == '\\') {
				if(!checkBlank() && !testEscapeValidity()) return false;
			}
	...
	private void generateWordToken(TokenType type) throws LexerException {
		...
		while(currentIndex < data.length){
			...
			if(state == LexerState.IN_DOCUMENT_TEXT && testEscapeValidity()){
				currentIndex++;
				word += data[currentIndex];
			}
			/*else if(state == LexerState.IN_STRING && testEscapeValidity()) {
				currentIndex++;
				word += data[currentIndex];
			}*/
			else if(!checkBlank()){
	...
	private boolean testEscapeValidity() {
		if(state == LexerState.IN_DOCUMENT_TEXT && data[currentIndex] == '\\'){
			if((currentIndex+1) >= data.length) throw new LexerException("Invalid escaping");
			if(data[currentIndex+1] == '{') return true;
			if(data[currentIndex+1] == '\\') return true;
			throw new LexerException("Invalid escaping");
		}
		else if(state == LexerState.IN_TAG && data[currentIndex] == '\\') {
			if((currentIndex+1) >= data.length) throw new LexerException("Invalid escaping");
			if(data[currentIndex+1] == '"') return true;
			if(data[currentIndex+1] == '\\') return true;
			throw new LexerException("Invalid escaping");
